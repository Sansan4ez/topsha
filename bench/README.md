...
- `hybrid` / `legacy_text`: combine algorithmic/routing checks with deterministic text assertions on the answer (`contains_any`, `regex`, `number`, `entity_url_pairs`) for cases where the final wording still matters. `entity_url_pairs` checks each requested entity and URL on the same answer line, preventing a global presence check from passing when links are swapped or an entity is omitted.

Reports expose selection accuracy, effective execution/fallback assertions, and answer correctness separately. This keeps a preserved selector identity from masking a fallback, while allowing a legitimate fallback with real evidence to pass. A document request with no document evidence must remain a partial/failure rather than pass because a route was selected.

### Route id convention (RFC-028)

`routing.route_id` in golden cases must be expressed in the **leaf-slug** form that `meta.retrieval_leaf_route_id` actually emits at runtime (e.g. `company_general`, `catalog_filters_by_category`), not the dotted `family.leaf` `route_id` used in the `core/routes/*/*.yaml` catalog (e.g. `corp_kb.company_common`, `corp_db.lamp_filters`). The two diverge because `meta.retrieval_route_id` can collapse siblings that share one physical KB scope (all `corp_kb.*` routes report `corp_kb.company_common` there). `route_id` is therefore the **selector identity** and must remain stable for selection accuracy; it is not necessarily the effective source/execution route after fallback. `routing_accuracy_summary` and `eval_routing` use the leaf slug for selection checks. Optional `routing.effective_route_id`, `effective_leaf_route_id`, `effective_source`, `used_fallback_route_id`, `used_fallback_scope`, and `evidence_status` assert the separately reported effective execution/fallback metadata. Golden routing cases may carry `routing.route_id_dotted` alongside `route_id` purely for human traceability back to the catalog file. If `core/documents/routing.py::ROUTE_BUSINESS_METADATA` changes a route's `leaf_route_id`, or a new route is added, update the corresponding golden `route_id` value(s) to match — `docker exec core sh -lc "curl ... /api/chat"` with `return_meta=true` against a live stack is the fastest way to read the current leaf slug back.
